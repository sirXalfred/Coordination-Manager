...
async function createNode(spec: WireframeNode, parent: FrameNode | PageNode): Promise<SceneNode> {
  let node: SceneNode
...
async function generateWireframes(spec: WireframeSpec) {
  ...
  for (let idx = 0; idx < spec.pages.length; idx++) {
    ...
    const rootNode = await createNode(pageSpec.frame, page) as FrameNode
    rootNode.x = xOffset
    rootNode.y = yOffset
...
figma.ui.onmessage = async (msg: { type: string; spec?: WireframeSpec }) => {
